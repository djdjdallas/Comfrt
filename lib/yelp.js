...
function parseSearchQuery(message, savedLocation = '') {
  ...
  // If still no location, return null - caller should prompt user
  if (!location) {
    location = null;
  }
...
export async function chatWithYelp(message, options = {}) {
  ...
  // If no location found, ask user to provide one
  if (!location) {
    return {
      message: "I'd love to help you find a comfortable spot! Could you tell me which city or neighborhood you're looking in? For example, try: 'Find quiet cafes in San Francisco' or 'cozy restaurants in Brooklyn'",
      venues: [],
      needsLocation: true,
    };
  }
  ...
  try {
    ...
    // Get top 6 businesses to fetch details for
    const topBusinesses = (data.businesses || []).slice(0, 6);

    // Fetch full business details AND reviews for each venue
    const venuesWithDetailsAndReviews = await Promise.all(
      topBusinesses.map(async (venue) => {
        try {
          // Fetch details and reviews in parallel for each venue
          const [detailsResponse, reviewsResponse] = await Promise.all([
            fetch(`${YELP_API_BASE}/businesses/${venue.id}`, {
              headers: { 'Authorization': `Bearer ${apiKey}` },
            }),
            fetch(`${YELP_API_BASE}/businesses/${venue.id}/reviews?limit=10&sort_by=yelp_sort`, {
              headers: { 'Authorization': `Bearer ${apiKey}` },
            })
          ]);
          ...
          // Analyze reviews with keyword-based analysis as baseline
          ...
          return {
            ...
            reviewAnalysis,
            comfortQuotes,
            venueDescription: buildVenueDescription(details)
          };
        } catch {
          // Silently handle errors - venue will use basic info
        }
        return venue;
      })
    );

    // Run Claude analysis in parallel for all venues (with actual reviews!)
    const venuesWithClaudeAnalysis = await Promise.all(
      venuesWithDetailsAndReviews.map(async (venue) => {
        // Send venue description AND reviews to Claude for intelligent analysis
