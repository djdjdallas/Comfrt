...
export async function chatWithYelp(message, options = {}) {
  ...
  try {
    ...
    // Fetch full business details for each venue (includes more data than search)
    const venuesWithDetails = await Promise.all(
      topBusinesses.map(async (venue) => {
        try {
          // Get full business details which may include more info
          const detailsResponse = await fetch(
            `${YELP_API_BASE}/businesses/${venue.id}`,
            {
          ...
          if (detailsResponse.ok) {
            ...
            // Merge details with venue
            return {
              ...venue,
              ...details,
              // Create pseudo-review from any available text
    ...
    const venuesWithClaudeAnalysis = await Promise.all(
      venuesWithDetails.map(async (venue) => {
        // Use venue description for Claude analysis (since we don't have reviews)
        if (venue.venueDescription) {
          const claudeAnalysis = await analyzeVenueWithClaude(venue);
