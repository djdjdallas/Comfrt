...
export async function chatWithYelp(message, options = {}) {
  ...
  try {
      ...
      topBusinesses.map(async (venue) => {
        try {
          ...
          if (reviewsResponse.ok) {
            ...
            // Log actual review text snippets for debugging
            if (reviews.length > 0) {
              console.log(`[chatWithYelp] ${venue.name} sample review:`, reviews[0].text?.substring(0, 100));
            }
            ...
            console.log(`[chatWithYelp] ${venue.name}: Analysis -`, {
              sentimentScore: reviewAnalysis.sentimentScore,
              confidence: reviewAnalysis.confidence,
              highlights: reviewAnalysis.highlights.map(h => h.text),
              totalMentions: reviewAnalysis.totalMentions
            });
    ...
    // Enhance venues with comfort scores (now with review data!)
    const venues = venuesWithReviews
      .map(venue => enhanceVenueWithReviews(venue, preferences))
...
/**
 * Generate response with actual review insights
 */
function generateResponseWithReviews(query, venues, location) {
  ...
  // Use actual review data for the response
  if (topVenue.reviewAnalysis?.highlights?.length > 0) {
    const highlight = topVenue.reviewAnalysis.highlights[0].text;
    response += `${topVenue.name} looks especially promising - reviewers describe it as "${highlight}".`;
