    ...
    if (businesses.length > 0) {
      ...
      const venuesWithDetails = businesses.slice(0, 6).map((business, index) => {
        ...
        if (reviews.length === 0 && mainSnippet) {
          reviews.push({
            id: 'snippet',
            text: mainSnippet.replace(/\[\[HIGHLIGHT\]\]/g, '').replace(/\[\[ENDHIGHLIGHT\]\]/g, ''),
        ...
        if (mainSnippet) {
          const cleanSnippet = mainSnippet.replace(/\[\[HIGHLIGHT\]\]/g, '').replace(/\[\[ENDHIGHLIGHT\]\]/g, '');
