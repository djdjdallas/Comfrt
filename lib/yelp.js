...
function buildVenueDescription(venue) {
  ...
  console.log(`[buildVenueDescription] ${venue.name}:`, description.substring(0, 200) + '...');
...
export async function chatWithYelp(message, options = {}) {
  ...
  try {
      ...
      topBusinesses.map(async (venue) => {
        try {
          ...
          if (detailsResponse.ok) {
            ...
            console.log(`[Details] ${venue.name}: Got details`, {
              hasHours: !!details.hours,
              hasAttributes: !!details.attributes,
            ...
            console.log(`[Details] ${venue.name}: Error ${detailsResponse.status}`);
          }
        } catch (err) {
          console.log(`[Details] Failed for ${venue.name}:`, err.message);
        }
        return venue;
      })
    );
    ...
    console.log(`[chatWithYelp] Running Claude analysis on ${venuesWithDetails.length} venues...`);
