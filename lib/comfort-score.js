...
export function generateRecommendationReason(venue, preferences = {}) {
  ...
  console.log(`[generateRecommendationReason] ${venue.name}:`, {
  ...
  // Only use "peaceful, quiet atmosphere" if we have actual data (not just inferred)
  if (venue.noise_level === 'quiet' && venue.reviewAnalysis?.breakdown?.noise?.mentions > 0) {
    reasons.push("Reviewers mention it's peaceful and quiet");
  }
  ...
  if (preferences.noiseSensitivity >= 4 && venue.comfort_score >= 70) {
    reasons.push("Should be comfortable for noise-sensitive visitors");
  }
  ...
  if (categories.includes('coffee') || categories.includes('cafe')) {
    reasons.push("Cafe setting - typically good for working or relaxing");
  }
  if (categories.includes('tea')) {
    reasons.push("Tea house - usually calm and peaceful");
  }
  if (venue.price === '$$$' || venue.price === '$$$$') {
    reasons.push("Upscale venue - typically quieter atmosphere");
  }

  const selectedReason = reasons.length > 0
    ? reasons[Math.floor(Math.random() * reasons.length)]
    : "A comfortable spot that should match your preferences";

  console.log(`[generateRecommendationReason] ${venue.name}: "${selectedReason}"`);
