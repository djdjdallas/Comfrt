...
export default function ComfortMap({ venues = [], initialCenter, onVenueClick }) {
  ...
  if (!MAPBOX_TOKEN) {
    return (
      <div style={{
        ...
        <p>Map requires Mapbox API key. Add NEXT_PUBLIC_MAPBOX_TOKEN to .env.local</p>
      </div>
    );
  }
      ...
      <div style={{
        ...
        {venue.rating && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
            ...
            <span style={{ fontSize: '13px', fontWeight: '500' }}>{venue.rating}</span>
          </div>
        )}

        {/* Price */}
        {venue.price && (
          <span style={{ fontSize: '13px', color: '#6b6b6b' }}>{venue.price}</span>
        )}
      </div>
